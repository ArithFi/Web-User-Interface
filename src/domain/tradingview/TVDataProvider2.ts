...
import {getChartPricesFromBinanceFutures, getCurrentPriceOfTokenFutures} from "../prices";
...
export class TVDataProvider2 {
  ...
  async getTokenHistoryBars(
    ...
    if (!barsInfo.data.length || barsInfo.ticker !== ticker || barsInfo.period !== period || shouldRefetchBars) {
      try {
        const bars = await getChartPricesFromBinanceFutures(ticker, period, 500);
        // @ts-ignore
  ...
  async getTokenLastBars(ticker: string, period: string, limit: number): Promise<Bar[]> {
    return getChartPricesFromBinanceFutures(ticker, period, limit);
  }
  ...
  async getCurrentPriceOfToken(ticker: string): Promise<number> {
    return getCurrentPriceOfTokenFutures(ticker);
  }
