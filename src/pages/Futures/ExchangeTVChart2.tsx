...
import { TVDataProvider2 } from "../../domain/tradingview/TVDataProvider2";
import { formatAmount, numberWithCommas } from "../../lib/numbers";
import { styled } from "@mui/material";
import {get24HrFromBinance, get24HrFromBinanceFutures} from "../../domain/prices";
...
const ExchangeTVChart: FC<ExchangeTVChartProps> = ({ ...props }) => {
  ...
  const fetchHr = useCallback(async () => {
    const data = await get24HrFromBinanceFutures(props.tokenPair);
    if (data) {
      setHr({
  ...
  useEffect(() => {
    // @ts-ignore
    dataProvider.current = new TVDataProvider2();
  }, []);
