...
const ReCharts: FC<ChartsProps> = ({...props}) => {
  ...
  const {data} = useSWR(`${serviceBaseURL(chainsData.chainId)}/dashboard/personal/asset?walletAddress=${props.address}&from=${from}&to=${to}&copy=1`,
    (url: string) => fetch(url, {
      ...
      .then((res) => res.map((item: any) => ({
        date: item.date,
        value: item?.copy_balance + item?.position_value_copy
      })))
  )
